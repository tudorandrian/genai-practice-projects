...
| A8 | Nothing unpublishable outside the working tree: git history, commit identities and pull-request descriptions. | ☐ | Pre-publication security and privacy review, 2026-09-16 (gitleaks over every ref, including pull-request refs: no secrets). Three findings block a visibility change of this repository. (1) Commit `d83dd75`, reachable from `main`, still contains four P02 fixtures (e06, e08, e09, e15) whose questions paraphrased course quiz questions; `599ee40` rewrote them, but history keeps the old text. (2) The descriptions of pull requests #6-#10, #13, #16 and #18 name course identifiers, the course data mirror and local tooling; GitHub keeps pull-request refs and description edit histories, which cannot be rewritten. (3) Every authored commit carries the author's institutional e-mail address. **Owner decision:** publish a new repository from a rewritten history (fixture blobs replaced, commit e-mail changed to the GitHub no-reply address), or record why each finding is accepted. Tick only after that decision is carried out. **Prepared:** [docs/publication.md](publication.md) describes the recommended route, and `scripts/prepare_public_history.py` builds and verifies the rewritten history. Dry run on 2026-09-16 against `main` at `3098731`: 23 commits rewritten, `verified: publishable`, final tree identical (`cbf562a`), gitleaks v8.28.0 "23 commits scanned. no leaks found". |
...
| C10 | Repository settings are complete: description, topics, MIT licence detected, private vulnerability reporting, secret scanning and push protection. | ☐ | Done as part of publication: the repository is still private, with no description or topics yet, and private vulnerability reporting and secret scanning can only be enabled once it is public. |

## Summary

Ticked: A1, A2, A3, A4, A5, A6, B. Unticked: A7 (the repository's own URLs need a public
repository), A8 (history and pull-request descriptions, an owner decision), and C8, C9, C10
(the owner's checks). No box was ticked without evidence recorded next to it.
...
- **Current state:** A1-A6 and B are ticked, and CI enforces A3 and the offline checks on every
  pull request. A7 waits for publication; A8 and C8-C10 wait for the owner.
...
| Date | Step | Evidence |
|---|---|---|
| 2026-09-16 | Release 1.0.0 merged: twelve projects, one squash-merged pull request each. | `4839850` (#17), `599ee40` (#18) |
| 2026-09-16 | Real-LLM tests, LangChain 1.x, licence notices and security policy merged. | `c3a861f` (#19) |
