...
| A5 | Every dataset in every README has a source and a licence; every `metrics.txt` is newer than its project's last code change. | ☑ | `release_check` → `PASS datasets-have-licences`. That check is a smoke test (the section exists and names a licence or provenance, in a table or in prose); per-dataset completeness is covered by B and C8. `metrics-fresh` fails when a proof is missing, or when a `uv run demo` run newer than the code changed a proof older than its project's last code commit. Such a stale proof passes when that demo run lists the project as `ok` and leaves its `output/` unchanged, and is reported as `skip` (with the action) when there is no such run. The substantive evidence is the A6 run: `uv run demo --all` followed by an empty `git status --porcelain -- projects/*/output`, so every committed proof is exactly what the code produces. Tests: `shared/tests/test_release_check.py::test_metrics_fresh_fails_only_when_the_proof_is_missing`, `::test_metrics_fresh_reports_a_stale_proof_as_skip_with_the_action_to_take`, `::test_metrics_fresh_passes_a_stale_proof_that_a_later_demo_run_reproduced`, `::test_metrics_fresh_fails_when_the_demo_changed_a_committed_proof`, `::test_metrics_fresh_still_skips_without_valid_demo_evidence`, `::test_demo_run_evidence_reads_the_summary_the_demo_runner_writes`, `::test_dataset_licence_check_does_not_match_mit_inside_an_ordinary_word`. |
...
| 2026-09-22 | Audit of `942af7b` closed: F1, F4, F5 and F7 fixed with regression tests (P11 demo kept out of the user's `data/`, P12 demo history kept separate, `demo --strict`, P12 stale-index refresh); F2, F3 and F6 documented as limits. Required checks green; the heavy run gave 12/12 ok under `--strict` on macOS and 11 ok + 1 P10 skip (known, A1) on Ubuntu. | Pull request #5 (`1623f41`); run [35787276308](https://github.com/tudorandrian/genai-practice-projects/actions/runs/35787276308) |
| 2026-09-23 | Version 1.2.0 prepared: `pyproject.toml`, `uv.lock`, `CITATION.cff`, `CHANGELOG.md`. The tag `v1.2.0` and GitHub release 1.2.0 follow the merge. | [CHANGELOG.md](../CHANGELOG.md) |
| 2026-09-23 | Tag `v1.2.0` on the release commit `69d4b4d` and GitHub release 1.2.0, with the 1.2.0 section of the changelog as notes. | [Release 1.2.0](https://github.com/tudorandrian/genai-practice-projects/releases/tag/v1.2.0) |
| 2026-09-23 | Fresh-clone fixes merged: P08 and P09 load pinned safetensors weights once, the demo runner names a missing dependency group, and `metrics-fresh` passes on a reproduced demo run. Required and heavy checks green. | `71eb6c8` (#9), `116c503` (#10) |
| 2026-09-23 | Version 1.2.1 prepared: `pyproject.toml`, `uv.lock`, `CITATION.cff`, `CHANGELOG.md`. The tag `v1.2.1` and GitHub release 1.2.1 follow the merge. | [CHANGELOG.md](../CHANGELOG.md) |
| 2026-09-23 | Tag `v1.2.1` on the release commit `cd854d9` and GitHub release 1.2.1, with the 1.2.1 section of the changelog as notes. | [Release 1.2.1](https://github.com/tudorandrian/genai-practice-projects/releases/tag/v1.2.1) |
| 2026-09-23 | LF proofs under every git line-ending setting: every text write pins `newline`, and `.gitattributes` checks the proofs out as LF. Fresh clones under `core.autocrlf` true, input and false leave 0 modified files after `uv run demo`. | #12 |
| 2026-09-23 | Version 1.2.2 prepared: `pyproject.toml`, `uv.lock`, `CITATION.cff`, `CHANGELOG.md`. The tag `v1.2.2` and GitHub release 1.2.2 follow the merge. | [CHANGELOG.md](../CHANGELOG.md) |
