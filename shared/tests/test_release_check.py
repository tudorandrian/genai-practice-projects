...
def test_large_file_check_exempts_uv_lock(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    # uv.lock is a generated lockfile the spec requires to be committed (D3); it is
    # exempt from the 100 KB rule by name, mirroring shared/tests/test_scaffold.py and
...
def test_metrics_fresh_compares_commit_history_not_mtime() -> None:
    # projects/p01_mini_etl's output/metrics.txt was committed in the same commit as
    # its code, so it must never be named as missing or stale, however file mtimes on
    # disk read right now (checkout order, not commit history).
    result = release_check.check_metrics_fresh(release_check.ROOT)
    assert "p01_mini_etl" not in result.detail


def test_metrics_fresh_fails_only_when_the_proof_is_missing(tmp_path: Path) -> None:
    # T15-d: a missing output/metrics.txt is the one unambiguous defect this check can
...
def test_metrics_fresh_reports_a_stale_proof_as_skip_with_the_action_to_take(
    ...
    # T15-d: a proof whose last commit predates the project's last code commit is real,
...
def test_dataset_licence_check_accepts_prose_without_a_table(tmp_path: Path) -> None:
    # T15-a: a project whose data needs no third-party licence because nothing was
...
def test_dataset_licence_check_does_not_match_mit_inside_an_ordinary_word(
    ...
    # B1 (task review): without a \b word boundary, "MIT" matches inside "committed"
...
def test_links_check_skips_localhost_and_example_hosts(
    ...
    # T15-b: loopback and RFC 2606 example domains are documentation illustrations, not
...
def test_links_check_treats_any_repo_subpath_404_the_same_way(
    ...
    # Fix round 2 (C1): the README now also links to Actions/commits/pulls under the
    # repo's own path, not just the bare repo URL — all of those 404 while private too.
    class _Resp:
        status_code = 404
